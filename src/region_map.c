...
static const u8 sMapHealLocations[][3] =
{
	...
	[MAPSEC_GATEWAY_ISLAND] = {MAP_GROUP(GATEWAY_ISLAND), MAP_NUM(GATEWAY_ISLAND), HEAL_LOCATION_GATEWAY_ISLAND},
    [MAPSEC_ARID_ISLAND] = {MAP_GROUP(ARID_ISLAND), MAP_NUM(ARID_ISLAND), HEAL_LOCATION_ARID_ISLAND},
    [MAPSEC_KAPOK_ISLAND] = {MAP_GROUP(KAPOK_ISLAND), MAP_NUM(KAPOK_ISLAND), HEAL_LOCATION_KAPOK_ISLAND},
    [MAPSEC_IGNEOUS_ISLAND] = {MAP_GROUP(IGNEOUS_ISLAND), MAP_NUM(IGNEOUS_ISLAND), HEAL_LOCATION_IGNEOUS_ISLAND},
};
...
static u8 ProcessRegionMapInput_Zoomed(void)
{
    ...
    if (JOY_HELD(DPAD_UP) && sRegionMap->zoomedCursorPosY > MAPCURSOR_Y_MIN)
    {
        sRegionMap->zoomedCursorDeltaY = -1;
        input = MAP_INPUT_MOVE_START;
    }
    if (JOY_HELD(DPAD_DOWN) && sRegionMap->zoomedCursorPosY < MAPCURSOR_Y_MAX)
    {
        sRegionMap->zoomedCursorDeltaY = +1;
        input = MAP_INPUT_MOVE_START;
    }
    if (JOY_HELD(DPAD_LEFT) && sRegionMap->zoomedCursorPosX > MAPCURSOR_X_MIN)
    {
        sRegionMap->zoomedCursorDeltaX = -1;
        input = MAP_INPUT_MOVE_START;
    }
    if (JOY_HELD(DPAD_RIGHT) && sRegionMap->zoomedCursorPosX < MAPCURSOR_X_MAX)
    {
        sRegionMap->zoomedCursorDeltaX = +1;
        input = MAP_INPUT_MOVE_START;
    }
